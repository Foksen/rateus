...
3. Настройте окружения и запуск проекта
   ...
   В приложении фронтенда создайте файл .env (`/frontend/rateus-app/.env`). Добавьте в него переменные из `/frontend/rateus-app/.env.local`
   ...
   Для запуска серверной части необходимо добавить переменные окружения из (/backend/core/.env.local) при запуске (Run configuration в IntelliJ IDEA)
   ...
   В приложении фронтенда создайте файл .env (`/frontend/rateus-app/.env`). Добавьте в него переменные из `/frontend/rateus-app/.env.example`
